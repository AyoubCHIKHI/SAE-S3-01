...
<body class="bg-gray-100 font-sans flex">
    ...
    <main class="flex-1 ml-64 p-8">
        <h1 class="text-2xl font-bold mb-6 text-gray-800">
            <?= $mode === 'create' ? 'Nouvel Evénement' : 'Modifier Evénement' ?></h1>
        ...
        <div class="bg-white rounded shadow p-6 max-w-2xl">
            ...
            <form action="<?= $mode === 'create' ? '/admin/evenements/store' : '/admin/evenements/update' ?>"
                method="POST">
                <?php if ($mode === 'edit'): ?>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="titre"
                        value="<?= htmlspecialchars($evenement['nom_'] ?? $data['titre'] ?? '') ?>"
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="date" name="date_debut"
                            value="<?= isset($evenement['date_debut']) ? date('Y-m-d', strtotime($evenement['date_debut'])) : '' ?>"
                            class="w-full border p-2 rounded" required>
                    </div>
                    <div>
                        <label class="block text-gray-700">Date Fin</label>
                        <input type="date" name="date_fin"
                            value="<?= isset($evenement['date_fin']) ? date('Y-m-d', strtotime($evenement['date_fin'])) : '' ?>"
                            class="w-full border p-2 rounded" required>
                    </div>
                </div>
