...
<div class="mb-6">
    ...
    <h2 class="text-3xl font-bold text-gray-800 mt-2">Modifier le Partenaire</h2>
</div>
...
<div class="bg-white rounded-lg shadow p-6 max-w-2xl">
    ...
    <form action="/admin/partenaires/edit?id=<?= $partenaire['id'] ?>" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <select name="type" required
                ...
                <option value="ENTREPRISE" <?= $partenaire['type'] === 'ENTREPRISE' ? 'selected' : '' ?>>Entreprise
                </option>
                <option value="FONDATION" <?= $partenaire['type'] === 'FONDATION' ? 'selected' : '' ?>>Fondation</option>
                <option value="INSTITUTION" <?= $partenaire['type'] === 'INSTITUTION' ? 'selected' : '' ?>>Institution
                </option>
            </select>
        </div>
        ...
        <div class="border-t pt-4 mt-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="contact_nom" value="<?= htmlspecialchars($partenaire['contact_nom']) ?>"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 border p-2">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">Email du contact</label>
                    <input type="email" name="contact_email"
                        value="<?= htmlspecialchars($partenaire['contact_email']) ?>"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 border p-2">
                </div>
            </div>
            <div class="mt-4">
                ...
                <input type="text" name="contact_telephone"
                    value="<?= htmlspecialchars($partenaire['contact_telephone']) ?>"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 border p-2">
            </div>
        </div>

        <div class="flex items-center mt-4">
            <input type="checkbox" name="est_grand_donateur" id="grand_donateur" <?= $partenaire['est_grand_donateur'] ? 'checked' : '' ?> class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
